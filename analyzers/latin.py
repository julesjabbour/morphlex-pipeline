...
def analyze_latin(word: str, return_all: bool = False) -> list[dict]:
    ...
    Queries both Morpheus REST API and LatMor, disambiguates to find the best
    parse, and returns confidence-ranked results.

    Args:
        word: Latin word to analyze
        return_all: If True, return all parses with best first. If False, return only best.

    Returns:
        List of dicts matching the lexicon.entries schema columns.
        When return_all=True, results are confidence-ranked with best parse first.
    """
    # Query both sources
    ...
    # Add remaining parses from Morpheus
    ...
    # Add remaining parses from LatMor
